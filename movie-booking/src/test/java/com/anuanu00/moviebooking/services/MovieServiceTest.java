...
import com.anuanu00.moviebooking.repositories.MovieRepository;
...
public class MovieServiceTest {
    ...
    public void getAllMovies_ShouldReturnAllMovies() {
        ...
        actualMovieList.add(new Movie("1", "MovieA",90));
        actualMovieList.add(new Movie("2", "MovieB",50));
        actualMovieList.add(new Movie("3", "MovieC",80));
    ...
    @Test
    @DisplayName("getAllMovies mrthod Should Return Empty List if No Movies")
    public void getAllMovies_ShouldReturnEmptyListIfNoMovies() {
        // Arrange
        List<Movie> actualMovieList = new ArrayList<>();
        when(movieRepositoryMock.getAllMovies()).thenReturn(actualMovieList);
        // Act
        List<Movie> expectedMovieLis = movieService.getAllMovies();
        // Assert
        Assertions.assertEquals(expectedMovieLis, actualMovieList);
        verify(movieRepositoryMock).getAllMovies();
